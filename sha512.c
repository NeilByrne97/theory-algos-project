...
#define WLEN 32    // Length of word
#define WORD uint32_t
#define PF PRIX32   // Print format - 32 bits HEX
#define BYTE uint8_t
// Page 5 of the Secure Hash Standard
#define ROTL(x,n) (x<<y)|(x>>(WLEN-n))
#define ROTR(x,n) (x>>y)|(x<<(WLEN-n))
#define SHR(x,n) x>>n
...
#define SIG0(x) ROTR(x, 2)^ROTR(x, 3)^ROTR(x, 22)
...
int sha512(FILE *f, WORD H[]){ // The function that performs the SHA256 alforithm on message f
